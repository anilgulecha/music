...
export function playLead(t, freq, dur, vel, P, rnd) {
  ...
  if (timbre === 'glass') {
  ...
  } else if (timbre === 'chime') { // ident chime: pure sine bell + a fast-dying 2.71× ping (ported from ident-lab)
    const lvl = 0.19 * vel;
    const o = A.ctx.createOscillator();
    ...
    g.gain.exponentialRampToValueAtTime(0.0001, t + 0.004 + Math.max(dur * 1.6, 1.2));
    o.connect(g); g.connect(bus); g.connect(A.nodes.echoIn);
    o.start(t); o.stop(t + dur * 1.6 + 1.4);
    const p = A.ctx.createOscillator();
    p.frequency.value = freq * 2.71;
    const pg = A.ctx.createGain();
    pg.gain.setValueAtTime(0.0001, t);
    pg.gain.exponentialRampToValueAtTime(lvl * 0.18, t + 0.002);
    pg.gain.exponentialRampToValueAtTime(0.0001, t + 0.002 + 0.35);
    p.connect(pg); pg.connect(bus);
    p.start(t); p.stop(t + 0.6);
    scrap(o, [g]); scrap(p, [pg]);
  } else if (timbre === 'marimba') { // struck bar: fast-decaying sine + brief 3.93× strike partial (ported from ident-lab)
    const lvl = 0.2 * vel;
    const o = A.ctx.createOscillator();
    o.frequency.value = freq; o.detune.value = detune;
    const g = A.ctx.createGain();
    ...
    g.gain.exponentialRampToValueAtTime(0.0001, t + 0.003 + 0.55);
    o.connect(g); g.connect(bus); g.connect(A.nodes.echoIn);
    o.start(t); o.stop(t + 0.9);
    const s = A.ctx.createOscillator();
    s.frequency.value = freq * 3.93;
    const sg = A.ctx.createGain();
    sg.gain.setValueAtTime(0.0001, t);
    sg.gain.exponentialRampToValueAtTime(lvl * 0.25, t + 0.002);
    sg.gain.exponentialRampToValueAtTime(0.0001, t + 0.002 + 0.08);
    s.connect(sg); sg.connect(bus);
    s.start(t); s.stop(t + 0.2);
    scrap(o, [g]); scrap(s, [sg]);
  } else if (timbre === 'bell') { // FM bell: 2.98× modulator with a decaying index (ported from ident-lab)
    const c = A.ctx.createOscillator();
    c.frequency.value = freq; c.detune.value = detune;
    const m = A.ctx.createOscillator();
    m.frequency.value = freq * 2.98;
    const mg = A.ctx.createGain();
    mg.gain.setValueAtTime(freq * 2.2, t);
    mg.gain.exponentialRampToValueAtTime(1, t + 1.1);
    m.connect(mg); mg.connect(c.frequency);
    const g = A.ctx.createGain();
    g.gain.setValueAtTime(0.0001, t);
    g.gain.exponentialRampToValueAtTime(0.19 * vel, t + 0.004);
    g.gain.exponentialRampToValueAtTime(0.0001, t + 0.004 + Math.max(dur * 1.5, 1.6));
    c.connect(g); g.connect(bus); g.connect(A.nodes.echoIn);
    c.start(t); m.start(t);
    c.stop(t + 3); m.stop(t + 3);
    scrap(c, [g, m, mg]);
  } else if (timbre === 'nylon') { // plucked string: saw through a fast-closing lowpass (ident-lab's 'pluck')
    const o = A.ctx.createOscillator();
    o.type = 'sawtooth';
    o.frequency.value = freq; o.detune.value = detune;
    const lp = A.ctx.createBiquadFilter();
    ...
    const g = A.ctx.createGain();
    g.gain.setValueAtTime(0.0001, t);
    g.gain.exponentialRampToValueAtTime(0.19 * vel * 0.9, t + 0.003);
    g.gain.exponentialRampToValueAtTime(0.0001, t + 0.003 + 0.7);
    o.connect(lp); lp.connect(g); g.connect(bus); g.connect(A.nodes.echoIn);
    o.start(t); o.stop(t + 1);
    scrap(o, [g, lp]);
  } else if (timbre === 'soft') { // rounded sustain: triangle through a fixed lowpass (ported from ident-lab)
    const o = A.ctx.createOscillator();
    o.type = 'triangle';
    o.frequency.value = freq; o.detune.value = detune;
    const lp = A.ctx.createBiquadFilter();
    ...
    const g = A.ctx.createGain();
    g.gain.setValueAtTime(0.0001, t);
    g.gain.linearRampToValueAtTime(0.18 * vel, t + 0.09);
    g.gain.setValueAtTime(0.18 * vel, t + Math.max(dur - 0.12, 0.1));
